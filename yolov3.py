...
def resnet50(class_num=10):
    ...
    return ResNet(ResidualBlock,
                  [3, 4, 6, 3], #layers
                  [64, 256, 512, 1024], #input shape
                  [256, 512, 1024, 2048], # shape
                  [1, 2, 2, 2], #stride
                  class_num)

class yolov3_resnet50(nn.Cell): #MODIFIED
    """
    ...
    def __init__(self, config):
        super(yolov3_resnet50, self).__init__()
        self.config = config
...
class yolov3_resnet18(nn.Cell): #MODIFY IT!!!!!!!!!!!!!!!!
    """
    ResNet based YOLOv3 network.
